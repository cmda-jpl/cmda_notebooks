...
var_name = season+'_'+data_info[0]['variable_name']    
dsp.write_netcdf(reference_dataset, path = workdir+domain.upper()+'_'+var_name+'_'+reference_name+'.nc')
for itarget, dataset in enumerate(target_datasets):
    dsp.write_netcdf(dataset, path = workdir+domain.upper()+'_'+var_name+'_'+target_names[itarget]+'.nc')
